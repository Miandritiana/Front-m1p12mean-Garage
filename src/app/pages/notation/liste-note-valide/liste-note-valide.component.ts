...
@Component({
  selector: 'app-liste-note-valide',
  standalone: true,
  imports: [ 
    RowComponent,
    ColComponent, 
  ...
  templateUrl: './liste-note-valide.component.html',
  styleUrl: './liste-note-valide.component.scss'
})
export class ListeNoteValideComponent {
  first:number=1;
  rows:number=25;
  totalRecords:number=1;
  dataSource: NoteConcoursDetailler[]= [];
  page=1;
  etat="";
  pageNumber=1;
  dateRange:Date[]=[];
  
  constructor(private noteService:NoteConcoursService,private router:Router, 
  ...
  public getData(){
    ...
    this.noteService.getEntretienValide(filtre).subscribe(
      data=> {
        this.loader.hide();
        if(data) {
